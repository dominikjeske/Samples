...
namespace HomeCenter.Model.Extensions
{
    ...
    {
        public static IObservable<TSource> DistinctForTime<TSource>(this IObservable<TSource> source,
        ...
        public static IObservable<TSource> DistinctForTime<TSource>(this IObservable<TSource> source,
            TimeSpan expirationTime, IScheduler scheduler)
        {
            return DistinctForTime(source, expirationTime, EqualityComparer<TSource>.Default, scheduler);
        }

        public static IObservable<TSource> DistinctForTime<TSource>(this IObservable<TSource> source,
            TimeSpan expirationTime, IEqualityComparer<TSource> comparer)
        {
                    ...
                    }, (state, item) => new
                    {
                        Acumulator = state.Acumulator
                            ...
                            .Concat(CheckForValueOrTimeout(expirationTime, state.Acumulator, item)
                                ? Enumerable.Repeat(
                                    new KeyValuePair<TSource, DateTimeOffset>(item.Value, item.Timestamp), 1)
                        ...
                        Next = CheckForValueOrTimeout(expirationTime, state.Acumulator, item)
                            ? Observable.Return(item.Value)
                            : Observable.Empty(item.Value)
                    }
                )
                .SelectMany(t => t.Next);
        }

        private static bool CheckForValueOrTimeout<TSource>(TimeSpan expirationTime,
            Dictionary<TSource, DateTimeOffset> state, Timestamped<TSource> item)
        {
        ...
        {
            return source.Select(message => Observable.FromAsync(() => onNextAsync(message)))
                .Concat()
                .Subscribe();
        }

        public static void Subscribe<T>(this IObservable<T> source, Func<T, Task> onNextAsync,
            Action<Exception> onError, CancellationToken cancellationToken, IScheduler observeOn)
        {
