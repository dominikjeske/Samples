...
namespace HomeCenter.EventAggregator.Handlers
{
    ...
    {
        public CommandHandler(Type type, Guid token, object handler, RoutingFilter filter) : base(type, token, handler,
            filter)
        {
        }
        ...
        public Task<R> HandleAsync<T, R>(IMessageEnvelope<T> message)
        {
            ...
            if (handler == null)
                throw new InvalidCastException(
                    $"Invalid cast from {Handler.GetType()} to Action<IMessageEnvelope<{typeof(T).Name}>>");
            handler(message);
