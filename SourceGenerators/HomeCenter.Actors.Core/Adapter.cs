...
namespace HomeCenter.Actors.Core
{
    public abstract class Adapter : DeviceActor
    {
        ...
        protected IList<string> RequierdProperties()
        {
            return _requierdProperties;
        }
            ...
            IDictionary<string, string> additionalProperties = null)
        {
            ...
            if (_requierdProperties.Count > 0)
                if (additionalProperties == null || additionalProperties.Count != _requierdProperties.Count ||
                    !_requierdProperties.SequenceEqual(additionalProperties.Keys))
                    throw new ArgumentException(
                        $"Update state on component {Uid} should be invoked with required properties: {string.Join(",", _requierdProperties)}");

            await MessageBroker.Publish(
                PropertyChangedEvent.Create(Uid, stateName, oldValue?.ToString(), newValue.ToString(),
                    additionalProperties), Uid);
            return newValue;
        }

        protected Task ScheduleDeviceRefresh(TimeSpan interval)
        {
            return MessageBroker.SendWithSimpleRepeat(ActorMessageContext.Create(Self, RefreshCommand.Default),
